...
export default function Index({ creators, filters }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Creator Directory" />
            <div className="flex h-full flex-1 flex-col gap-8 p-6">
                <div className="text-center">
                    <h1 className="text-4xl font-bold">Discover Creators</h1>
                    <p className="mt-2 text-lg text-muted-foreground">
                        Partner with talented creators to grow your brand
                    </p>
                </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Creator name or description..."
                            ...
                            <div>
                                <label className="mb-2 block text-sm font-medium">Follower Age Range</label>
                ...
                {creators.data.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-sidebar-border/70 bg-white p-12">
                        ...
                        <h2 className="mb-2 text-2xl font-bold">No Creators Yet</h2>
                        <p className="text-center text-muted-foreground">
                            Check back soon! We're onboarding talented creators every day.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        {creators.last_page > 1 && (
                            <div className="flex items-center justify-center gap-2">
                                {creators.current_page > 1 && (
                                    <Link
                                        href={`/creators?page=${creators.current_page - 1}`}
                                ...
                                <div className="flex gap-1">
                                    {Array.from({ length: Math.min(creators.last_page, 5) }, (_, i) => {
                                        let page;
                                        if (creators.last_page <= 5) {
                                            page = i + 1;
                                        } else if (creators.current_page <= 3) {
                                            page = i + 1;
                                        } else if (creators.current_page >= creators.last_page - 2) {
                                            page = creators.last_page - 4 + i;
                                        } else {
                                            page = creators.current_page - 2 + i;
                                        }

                                        return (
                                            <Link
                                                key={page}
                                                href={`/creators?page=${page}`}
